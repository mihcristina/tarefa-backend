...
export const getTask = async(request: Request, response: Response) => {
    ...
    const task = await getRepository(Tasks).findOne({where: [{id: Number(id)}]})
    return response.json(task);
};
export const updateTask = async(request: Request, response: Response) => {
    ...
    if (task.affected == 1){
        const taskUpdated = await getRepository(Tasks).findOne({where: [{id: Number(id)}]})
...
export const finishedTask = async(request: Request, response: Response) => {
    ...
    if (task.affected == 1){
        const taskFinished = await getRepository(Tasks).findOne({where: [{id: Number(id)}]})
